...
//            }
    ...
    private static void writeToArray(int height, TreeNode TreeNode, String[][] r, int i, int j) {
        if (TreeNode == null) {
            return;
        }
        r[i][j] = String.valueOf(TreeNode.val);
        int curLevel = (i + 1) >> 1;
        if (curLevel == height) {
            return;
        }
        ...
        if (TreeNode.left != null) {
            r[i + 1][j - gap] = "/";
            writeToArray(height, TreeNode.left, r, i + 2, j - gap * 2);
        }
        if (TreeNode.right != null) {
            r[i + 1][j + gap] = "\\";
            writeToArray(height, TreeNode.right, r, i + 2, j + gap * 2);
        }
    }
